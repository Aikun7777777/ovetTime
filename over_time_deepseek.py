...
class OvertimeParser:
    def __init__(self, api_key="sk-0aaaf65d35980f98fd", base_url="https://api.deepseek.com"):          
    # def __init__(self, api_key="sk-proj-I", base_url="https://api.openai.com"):      
        ...
        while incomplete:
            ...
            try:
                response = self.client.chat.completions.create(
                    ...
                    messages=[{
                        "role": "system", 
                        "content": "你是一个专门提取加班信息的助手，只返回JSON格式的解析结果，不返回其他任何内容。"
                    }, {
                        "role": "user", 
                        "content": prompt
                    }],
            ...
            try:
                result_text = response.choices[0].message.content.strip()
                print("返回的解析结果：", result_text)  # 打印返回的内容以调试
                # 尝试解析JSON
                try:
                    partial_result = json.loads(result_text)
                    ...
                    print(f"返回的文本内容: {result_text}")  # 打印返回的文本内容
                    text = input("请输入补充内容：")
            except Exception as e:
                print(f"API调用失败: {str(e)}")
                return {"error": f"API调用失败: {str(e)}"}
...
if __name__ == "__main__":
    app.run(debug=True, host='192.168.3.156', port=5000)
